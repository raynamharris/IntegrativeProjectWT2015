...
filtered <- slim1 # subsequently in this analysis, I use this line to filter rows 
exp_factors <- filtered[,c(1:3)] # extract experimental design variables
exp_nums <- filtered[,c(4:42)]

# Anova
#for(y in names(exp_nums)){
#  ymod<- summary(aov(exp_nums[[y]] ~ exp_factors$APA2 * exp_factors$TrainSessionCombo ))
#  cat(paste('\nDependent var:', y, '\n'))
#  print(ymod)
#}
...
filtered <- slim1 %>% filter(TrainSessionCombo == "T4_C1", APA != "control") 
...
slim1 <- behavior[,c(15,16,14,20:58)]
filtered <- slim1 %>% filter(TrainSessionCombo == "T6_C3", APA != "control") 
exp_factors <- as.data.frame(filtered[,1])
exp_nums <- filtered[,c(4:42)]
exp_factors$APA2 <- factor(filtered$APA2, levels = c("consistent", "conflict"))
