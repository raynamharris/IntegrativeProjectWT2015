...
levels(pcadata$APA2) <- c("yoked consistent","consistent",  "yoked conflict","conflict")
...
PCA12 <- ggplot(pcadata, aes(pcadata$PC1, pcadata$PC2, color=Punch)) +
    geom_point(size=1.5, aes(shape=APA2), alpha = 0.8) +
    ...
    scale_shape_manual(values=c(1, 16, 0, 15), aes(color=colorvalPunch))
PCA12


PCA42 <- ggplot(pcadata, aes(pcadata$PC5, pcadata$PC2)) +
    geom_point(size=1.5, aes(color=Punch, shape=APA2), alpha = 0.8) +
  ...
  scale_x_continuous(limits=c(-10, 15)) +
        scale_y_continuous(limits=c(-15, 15)) +
    scale_shape_manual(aes(color=colorvalPunch), values=c(1, 16, 0, 15)) +
...
pcaplots <- plot_grid(plotnolegend, legend, ncol = 2, rel_widths  = c(1, .3))
pcaplots
