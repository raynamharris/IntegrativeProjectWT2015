...
PCA42 <- ggplot(pcadata, aes(pcadata$PC5, pcadata$PC2)) +
    geom_point(size=1.5, aes(shape=subfield, color=treatment), alpha = 0.8) +
  ...
  scale_x_continuous(limits=c(-10, 15)) +
        scale_y_continuous(limits=c(-15, 15)) +
    scale_shape_manual(aes(color=colorvalAPA00), values=c(1, 16, 0, 15)) +
...
plotnolegend <- plot_grid(PCA12 + theme(legend.position = "bottom"), 
                          PCA42 + theme(legend.position = "bottom"),
           labels = c("B", "C"),
           nrow = 1,
           label_size = 7
           )
