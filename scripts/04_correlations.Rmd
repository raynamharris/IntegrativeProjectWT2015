...
vsdDG <- combinePCvsd("../data/03_DG_vsdtraining.csv", "DG", everything())
vsdCA1 <- combinePCvsd("../data/03_CA3_vsdtraining.csv", "CA1", everything())
vsdCA3 <- combinePCvsd("../data/03_DG_vsdtraining.csv", "CA3", everything())
...
## Correlate DG genes with PC1 and PC2

```{r makecorrrmat}

# I'm using the select function to get PC1, PC1, and all the genes. Use `tail(names(vsdDG),5)` to see that last genes is "ZZZ3"

vsdDG <- vsdDG %>%
  select(-trial, -trialNum)
head(names(vsdDG),10)
tail(names(vsdDG),5)


forcorall <-  vsdDG %>% select(PC1:Zzz3)	# gene gene ames
...
```{r correlations, fig.width=6.69, fig.height=3.5, eval= F}
...
write.csv(allvsd, "../data/for3Dplot.csv", row.names = F)
```
