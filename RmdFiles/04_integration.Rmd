...
alldata$avoidance <-  ifelse(grepl("Control", alldata$APA), "no", "yes")



scatter <- ggplot(alldata, aes(Behavior_PC1, as.numeric(CA1_PC3), color=avoidance)) + 
  geom_point(size = 2) +
...
pdf(file="../figures/02d_rnaseqavoidance/scatter1.pdf", width=1.5, height=1.5)
plot(scatter)
dev.off()



scatter <- ggplot(alldata, aes(x = Late_Potentiation, as.numeric(CA1_PC1), color=avoidance)) + 
...
pdf(file="../figures/02d_rnaseqavoidance/scatter3.pdf", width=1.5, height=1.5)
plot(scatter)
dev.off()

scatter <- ggplot(alldata, aes(as.numeric(DG_PC2), as.numeric(CA1_PC3), color=avoidance)) + 
  geom_point(size = 2, alpha = 0.5) +
  scale_color_manual(values = colorvalavoidance) +
    theme_cowplot(font_size = 8, line_size = 0.25)  +
    theme(legend.position="none") +
...
pdf(file="../figures/02d_rnaseqavoidance/scatter4.pdf", width=1.5, height=1.5)
plot(scatter)
dev.off()
```
