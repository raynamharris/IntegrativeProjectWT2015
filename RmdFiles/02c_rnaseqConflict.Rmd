...
colData <- colData %>% 
  filter(APA2 %in% c("conflict", "yoked_conflict" ))  %>% 
  droplevels()
...
dds$APA2 <- factor(dds$APA2, levels=c("conflict", "yoked_conflict" )) ## specify the factor levels
...
pheatmap(DEGes, show_colnames=F, show_rownames = F,
         ...
         breaks=myBreaks,
         clustering_distance_cols="correlation",
         filename = "../figures/02c_RNAseqConflict/pheatmap.pdf"
         )
...
pdf(file="../figures/02c_RNAseqConflict/PCA12.pdf", width=2.5, height=2.25)
plot(PCA12)
dev.off()

pdf(file="../figures/02c_RNAseqConflict/PCA62.pdf", width=2.5, height=2.25)
plot(PCA62)
dev.off()
...
pheatmap(DEGes, show_colnames=F, show_rownames = F,
         ...
         breaks=myBreaks,
         clustering_distance_cols="correlation", 
         filename = "../figures/02c_RNAseqConflict/pheatmap_minimal.pdf"
         )
```
...
pdf(file="../figures/02c_RNAseqConflict/AllCA1DG.pdf", width=1.5, height=1.75)
plot(colored)
dev.off()
...
colored <- ggplot(data, aes(x = lfc, y = pvalue)) + 
  ...
  geom_hline(yintercept = 1.3,  size = 0.25, linetype = 2 ) 
colored
pdf(file="../figures/02c_RNAseqConflict/AllCA1CA3.pdf", width=1.5, height=1.75)
plot(colored)
dev.off()
...
colored
#cvd_grid(colored)
pdf(file="../figures/02c_RNAseqConflict/AllDGCA3.pdf", width=1.5, height=1.75)
plot(colored)
dev.off()
...
#cvd_grid(colored) # to view plot for color blind 
pdf(file="../figures/02c_RNAseqConflict/Allconsistentconflict.pdf", width=1.5, height=1.75)
plot(colored)
dev.off()
...
dds$APA2 <- factor(dds$APA2, levels=c("consistent", "conflict")) ## specify the factor levels
...
pdf(file="../figures/02c_RNAseqConflict/DGconsistentconflict.pdf", width=1.5, height=1.75)
plot(colored)
dev.off()
...
pdf(file="../figures/02c_RNAseqConflict/CA3consistentconflict.pdf", width=1.5, height=1.75)
plot(colored)
dev.off()
...
#cvd_grid(colored) # to view plot for color blind 
pdf(file="../figures/02c_RNAseqConflict/CA1consistentconflict.pdf", width=1.5, height=1.75)
plot(colored)
dev.off()
...
sign[res$log2FoldChange<0]=-1  
logs$logP <- logs$logP*sign
write.csv(logs, file = "./02e_GO_MWU/CA1conflictconsistent.csv", row.names = F)
...
```{r data2forheatmap, message=F, warning=F, echo=F}
colData <- read.csv("../data/02a_colData.csv", header = T)
countData <- read.csv("../data/02a_countData.csv", header = T, check.names = F, row.names = 1)
...
dds$Punch <- factor(dds$Punch, levels=c("DG","CA3", "CA1")) ## specify the factor levels
dds$APA2 <- factor(dds$APA2, levels=c("yoked_consistent" ,"yoked_conflict", "consistent", "conflict")) 
...
contrast2 <- resvals(contrastvector = c("Punch", "CA1", "CA3"), mypval = 0.05) # 2122
contrast3 <- resvals(contrastvector = c("Punch", "CA3", "DG"), mypval = 0.05) # 2122
contrast4 <- resvals(contrastvector = c("APA2", "consistent", "conflict"), mypval = 0.05) # 0
contrast5 <- resvals(contrastvector = c("APA2", "consistent", "yoked_consistent"), mypval = 0.05) # 84
contrast6 <- resvals(contrastvector = c("APA2", "conflict", "yoked_conflict"), mypval = 0.05) # 15
contrast7 <- resvals(contrastvector = c("APA2", "yoked_conflict", "yoked_consistent"), mypval = 0.05) # 12


DEGes <- assay(rld)
DEGes <- cbind(DEGes, contrast1, contrast2, contrast3, contrast5, contrast6, contrast7)
DEGes <- as.data.frame(DEGes) # convert matrix to dataframe
DEGes$rownames <- rownames(DEGes)  # add the rownames to the dataframe
DEGes$padjmin <- with(DEGes, pmin(padjPunchCA1DG, padjPunchCA1CA3, padjPunchCA3DG, padjAPA2yoked_conflictyoked_consistent, padjAPA2conflictyoked_conflict, padjAPA2consistentyoked_consistent)) 
head(DEGes)
...
ann_colors <- ann_colors4 #see figure options to change colors
...
pheatmap(DEGes, show_colnames=F, show_rownames = F,
         ...
         clustering_distance_cols="correlation",
         annotation_legend = FALSE, 
         filename = "../figures/02c_RNAseqConflict/pheatmapAll.pdf"
         )
