...
speedsummary <- read.csv("../data/01a_speedsummary.csv", header = T)
timecw <- read.csv("../data/01a_timecw.csv", header = T)
...
behaviorsummaryNum$APA <- factor(behaviorsummaryNum$APA, levels = c("control",  "conflict","consistent", "conflict"))
...
levels(behaviorsummaryNumAPA2$APA2) #confirm factor levels set
speedsummary$APA2 <- factor(speedsummary$APA2, levels = c(  "yoked-consistent" ,"yoked-conflict", "consistent", "conflict"))
timecw$APA2 <- factor(timecw$APA2, levels = c( "yoked-consistent" , "yoked-conflict", "consistent",  "conflict"))
...
```{r speedccw}
#mean and sem for two variables
timecwplot <- ggplot(timecw, aes(x=, TrainSessionComboNum, y=m, color=APA2)) + 
    geom_errorbar(aes(ymin=m-se, ymax=m+se, color=APA2), width=.1) +
    geom_point(size = 2) +
    geom_line() +
    scale_y_continuous(name="Time spent CCW") +
    scale_x_continuous(name = NULL, 
                       breaks = c(1, 2, 3, 4, 5, 6, 7, 8, 9),
                       labels = NULL) +
  theme_cowplot(font_size = 8, line_size = 0.25) +
...
pdf(file="../figures/01_behavior/timecwplot.pdf",width=3.25, height=1.25)
plot(timecwplot)
dev.off()

speedplot <- ggplot(speedsummary, aes(x=, TrainSessionComboNum, y=m, color=APA2)) + 
    geom_errorbar(aes(ymin=m-se, ymax=m+se, color=APA2), width=.1) +
    geom_point(size = 2) +
   geom_line() +
...
pdf(file="../figures/01_behavior/speedplot.pdf",width=3.25, height=1.25)
plot(speedplot)
dev.off()

numentrance <- ggplot(behaviorsummaryNumAPA2, aes(x=, TrainSessionComboNum, y=m, color=APA2)) + 
    geom_errorbar(aes(ymin=m-se, ymax=m+se, color=APA2), width=.1) +
    geom_point(size = 2) +
   geom_line() +
    scale_y_continuous(name="Number of Entrances") +
...
pdf(file="../figures/01_behavior/numentrance.pdf",width=3.25, height=1.25)
plot(numentrance)
dev.off()
...
onebehavior(data=behavior, 
                  xcol="TrainSessionComboNum", ycol="Speed1",
                  yaxislabel="Speed1",
                  colorcode="APA")
