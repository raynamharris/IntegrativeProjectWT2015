...
prettyvenn <- venn.diagram(
  ...
  cat.dist = c(0.05, 0.05, 0.05), cat.pos = 1,
...
pdf(file="../figures/02i_RNAseqCA1/consistent.pdf", width=1.25, height=1.75)
plot(colored)
dev.off()
...
pdf(file="../figures/02i_RNAseqCA1/consistent2.pdf", width=1.5, height=1.75)
plot(colored)
dev.off()
...
pdf(file="../figures/02i_RNAseqCA1/confict.pdf", width=1.25, height=1.75)
plot(colored)
dev.off()
...
pdf(file="../figures/02i_RNAseqCA1/yoked.pdf", width=1.25, height=1.75)
plot(colored)
dev.off()
...
res <- results(dds, contrast =c("APA2",  "conflict", "yoked_conflict"), independentFiltering = T, alpha = 0.05)
resOrdered <- res[order(res$padj),]
head(resOrdered, 10)
summary(res)
topGene <- rownames(res)[which.min(res$padj)]
...
colored <- ggplot(data, aes(x = lfc, y = pvalue)) + 
  ...
  theme(panel.grid.minor=element_blank(),
        panel.grid.major=element_blank()) + 
  scale_x_continuous(name="log2 (conflict/yoked)",
                     limits=c(-4, 4)) +
  scale_y_log10(limits=c(0.00001, 10)) +
  labs( y = NULL) +
  geom_hline(yintercept = 1.3,  size = 0.25, linetype = 2)
...
#cvd_grid(colored) # to view plot for color blind 
pdf(file="../figures/02i_RNAseqCA1/conflictyoked.pdf", width=1.25, height=1.75)
plot(colored)
dev.off()
